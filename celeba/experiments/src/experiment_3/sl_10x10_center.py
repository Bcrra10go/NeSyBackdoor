...
CONFIG = {
    ...
    'trigger_size': 10,  # Size of the trigger pattern
...
def setup_experiment():
    ...
    base_dir = os.path.join(workspace_root, 'reports', 'experiment_3')
...
class PoisonedSemiSupervisedCelebA(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if idx in self.poison_indices:
            # Calculate center position for the trigger
            center_x = (CONFIG['image_size'] - CONFIG['trigger_size']) // 2
            center_y = (CONFIG['image_size'] - CONFIG['trigger_size']) // 2
            
            # Add trigger pattern at the center
            image = image.clone()
            image[:, center_y:center_y + CONFIG['trigger_size'], center_x:center_x + CONFIG['trigger_size']] = self.trigger
